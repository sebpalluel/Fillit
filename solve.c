...
/*   Updated: 2017/01/21 18:43:57 by psebasti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fillit.h"

size_t		solve(t_list **tet, size_t numtetri)
{
	t_map	**map = NULL;
	size_t	size;
	size_t	unsolved;
...
size_t		evaluate_new_pos_tetri(t_map **map, t_list **tet, unsigned int *pos_coord, size_t pos)
{
	size_t			new_pos;
	size_t			width;
	size_t			height;
...
int			backtracker(char *result, t_map **map, t_list **tet, unsigned int *coord, \
		int erase)
{
	size_t		pos;
	...
	while (pos < ft_strlen(result))
	{
		...
		{ 
			...
			*tet = (*tet)->next;
			if (backtracker(result, map, tet, coord, erase) == 0)
				return (0);
		}
		pos++;
	}
